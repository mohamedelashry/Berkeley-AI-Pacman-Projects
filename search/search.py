...
def generalGraph(problem, structure):
    """
...
def depthFirstSearch(problem):
    ...
    return generalGraph(problem, stack)
...
def breadthFirstSearch(problem):
    ...
    return generalGraph(problem, queue)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    Astar_cost = lambda path: problem.getCostOfActions([x[1] for x in path][1:]) + heuristic(path[-1][0], problem)

    # Construct an empty priority queue that sorts using f(x)
    pq = util.PriorityQueueWithFunction(Astar_cost)
